...
const char * const BITCOIN_CONF_FILENAME = "eclipsim.conf";
const char * const BITCOIN_PID_FILENAME = "eclipsimd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "eclipsim" is a composite category enabling all Eclipsim-related debug output
                if(ptrCategory->count(std::string("eclipsim"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Eclipsim
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Eclipsim
    // Mac: ~/Library/Application Support/Eclipsim
    // Unix: ~/.eclipsim
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Eclipsim";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Eclipsim";
#else
    // Unix
    return pathRet / ".eclipsim";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty eclipsim.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override eclipsim.conf
